...
| 维度 | 决定 |
|---|---|
| **实现栈** | Python + PyTorch + MPS（吃满 Apple Silicon Mac 的 Apple GPU；Phase 2 扩散生态也在 PyTorch） |
...
| 01 | [architecture](./01-architecture.md) | 目标 Python 包结构、模块职责、数据流、MPS/统一内存、parity 决策 |
...
| 08 | [quickstart](./08-quickstart.md) | Apple Silicon 上手：安装 → 预拉取 → smoke → 出片 |
...
- [ ] **在 Apple Silicon Mac（任意 M 系列 / MPS）上的真机验证**：扩散画质、时序稳定性、性能基准（需用户侧执行）
...
3. 实现与单测可在任意环境完成；**真机性能基准需在你的 Apple Silicon Mac（任意 M 系列 / MPS）上执行**（本 CI 环境无 GPU）。
